...
if target_path.is_file():
    ...
    local_data = {}
legacy = {'ecc'}
repo_data = {key: value for key, value in repo_data.items() if key not in legacy}
removed = sorted(key for key in legacy if key in local_data)
...
changed = local_data.pop('ecc', None) is not None
for key, value in repo_data.items():
    if key != 'ecc' and key not in local_data:
