...
setup.sh 是幂等的，可安全重复运行；它会自动串起 submodules、RTK、ECC、context-mode、OMC、superpowers、settings.json 和验证。支持 `--ci`、`--dry-run`、`--no-claude`、`--no-verify`、`--smoke-test`、`--ecc-focused`、`--ecc-full`、`--ecc-profile`、`--ecc-modules`。
...
| install-ecc.sh | 普通用户路径列出 ECC 可选安装范围并等待用户显式选择；CI 或 `--ecc-full` 使用 full profile；并叠加自定义 agents 覆盖 |
...
普通用户运行 `./setup.sh` 时，脚本不会预设 ECC 安装范围；它会列出可选安装方式，并跳过 ECC 官方安装器，直到用户显式选择：

- `./setup.sh --ecc-focused`：安装本仓库推荐常用开发能力
...
- `./setup.sh --ecc-profile <minimal|core|developer|security|research|full>`：安装官方 profile
...
~/.claude/skills/   ← 显式选择后安装的 ECC skills；OMC/superpowers skills 由 plugin marketplace 发现
...
- **ECC** — 普通用户路径只列出可选安装范围；使用 `--ecc-focused`、`--ecc-full`、`--ecc-profile` 或 `--ecc-modules` 后才执行官方安装器
- **OMC / context-mode** — 各自按官方安装入口执行；OpenSpec 使用官方全局 npm 安装
