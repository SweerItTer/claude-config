...
struct UiState {
  int tab_selected = 0;     // 0=Install 1=Update 2=Uninstall 3=诊断 4=Agents Skills
...
Component MakeTabIndicator(UiState* state) {
  return Renderer([state] {
    Elements cells;
    const char* names[] = {"1 Install", "2 Update", "3 Uninstall", "4 诊断", "5 Agents Skills"};
    for (int i = 0; i < 5; ++i) {
...
int RunUi(const std::filesystem::path& repo_root, const Manifest& manifest,
  ...
  // ---- Agents Skills 页：只安装 skills 到 ~/.agents/skills/（非 claude 目标只支持 skills）----
  // 复用外部 skills + 本地 skills 的勾选状态（与 Install 页同源，该页不涉及 plugins）。
  // 未勾选 = 全量；勾选 = 指定安装（外部 --skill / 本地 --update-local-skill）。
  Components agents_local_cols, agents_skill_cols;
  for (size_t i = 0; i < runner.local_skill_ids.size(); ++i) {
    agents_local_cols.push_back(Checkbox("[local] " + runner.local_skill_ids[i],
                                     &runner.local_selected[i].value));
  }
  for (size_t i = 0; i < manifest.skills.size(); ++i) {
    agents_skill_cols.push_back(Checkbox(manifest.skills[i].name + "  " +
                                         TruncateUtf8(manifest.skills[i].note, 36),
                                     &runner.skills_selected[i].value));
  }
  auto agents_local_col = Container::Vertical(std::move(agents_local_cols));
  auto agents_skill_col = Container::Vertical(std::move(agents_skill_cols));
  Components agents_cols;
  if (!runner.local_skill_ids.empty()) agents_cols.push_back(agents_local_col | flex);
  if (!manifest.skills.empty()) agents_cols.push_back(agents_skill_col | flex);
  if (agents_cols.empty()) {
    // 空页：无 skills 可选时给占位，避免 Empty container 吞焦点
    agents_cols.push_back(Renderer([] { return text("（无 skills 可安装）") | dim; }));
  }
  auto agents_page = Container::Vertical({
      Container::Horizontal(std::move(agents_cols)) | flex,
      Renderer([&] {
        return text(" 只安装 skills → ~/.agents/skills/（外部 npx -a universal -g + 仓库自有 "
                    "symlink；不装 claude 配置/plugins）") | dim;
      }),
  });
  ...
  auto modal_ok = Button("执行", [&] {
    ...
    switch (state.tab_selected) {
      ...
      case 4: {
        // Agents Skills：只装 skills 到 ~/.agents/skills/（非 claude 目标只支持 skills）
        std::vector<std::string> args = {"install", "--agents=universal", "--ci"};
        if (runner.SelectedSkillCount() > 0 || runner.SelectedLocalCount() > 0) {
          for (size_t i = 0; i < runner.local_skill_ids.size(); ++i)
            if (runner.local_selected[i].value) {
              args.push_back("--update-local-skill");
        ...
        runner.Spawn(std::move(args), "安装 Agents skills");
        break;
      }
    }
  });
  ...
  auto tabs = Container::Tab(
      {install_page_full, update_page, uninstall_page, diagnose_page, agents_page},
      &state.tab_selected);
      ...
      [&](Event e) {
        ...
        if (e == Event::Character('a') || e == Event::Character('A')) {
          if (state.tab_selected == 0) {
            ...
            // Agents Skills 页：只全选外部 + 本地 skills，不涉及 plugins
            const bool any_unchecked =
                std::any_of(runner.skills_selected.begin(),
                            runner.skills_selected.end(),
                            [](const BoolSlot& s) { return !s.value; }) ||
            ...
            case 4:
              state.confirm_text =
                  runner.SelectedSkillCount() == 0 && runner.SelectedLocalCount() == 0
                      ? "未勾选任何项 — 将全量安装所有 skills 到 ~/.agents/skills/，确认？"
                      : "安装选中的 skills 到 ~/.agents/skills/（非 claude 目标不装 claude 配置/plugins），确认？";
              break;
          }
          return true;
        }
